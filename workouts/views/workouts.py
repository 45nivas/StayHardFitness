...
- "3 sets of 30kg, 25kg, 20kg" → 3 separate sets with different weights
- "set 1 30kg 8 reps, set 2 25kg 6 reps" → parse each set individually
- "first set 30kg, second set 25kg" → set 1 and set 2
- If all sets same weight → repeat that weight for each set
- If only total sets mentioned with one weight → use same weight for all sets

SPOTTER DETECTION:
- "with spot", "with spotter", "spotted", "with help" → with_spotter=true for that set
- "without spot", "unassisted" → with_spotter=false

FAILURE DETECTION:
- "till failure", "to failure", "failed" → to_failure=true, reps=0 for that set
- If reps are not mentioned or missing in a set → to_failure=true, reps=0 for that set

WEIGHT RULES:
- Last number before kg/lbs is always the weight
- "each hand/arm/side" → that is the per-hand weight, use it as weight directly
- "bodyweight" or "bw" → weight=0
- CRITICAL: weight is ALWAYS the number paired with a unit like kg/lbs/pounds.
  NEVER use the reps number as weight.
  If input is "15 reps 50kg" → weight=50, reps=15 (NOT weight=15)
  If input is "3 sets 15 reps 50kg" → weight=50, reps=15
  When in doubt: find the unit first (kg/lbs), then take the number before it.

MUSCLE GROUP MAPPING:
squats, leg press, lunges, romanian deadlift, calf raises → "Legs"
bench press, incline, decline, chest fly, push ups, dips → "Chest"
...
tricep pushdown, skull crusher, tricep extension → "Triceps"
anything else → "General"

CASUAL SPEECH:
Ignore all filler: "bro", "man", "yaar", "then", "after that", 
"finished with", "started with", "with spot", emotions, 
and non-exercise words.

EXAMPLES:
"bench press 3 sets, first set 30kg 8 reps, second set 25kg 6 reps 
with spot, third set 20kg 4 reps till failure"
→ {
  "name": "Bench Press",
  "muscle_group": "Chest",
  "sets": [
    {"set_number": 1, "reps": 8, "weight": 30, "unit": "kg", 
     "with_spotter": false, "to_failure": false, "notes": ""},
    {"set_number": 2, "reps": 6, "weight": 25, "unit": "kg", 
     "with_spotter": true, "to_failure": false, "notes": "with spotter"},
    {"set_number": 3, "reps": 4, "weight": 20, "unit": "kg", 
     "with_spotter": false, "to_failure": true, "notes": "failure set"}
  ]
}

"pull ups 4 sets, first 3 sets bodyweight 10 reps, last set till failure with spot"
→ {
  "name": "Pull Ups",
  "muscle_group": "Back",
  "sets": [
    {"set_number": 1, "reps": 10, "weight": 0, "unit": "kg", 
     "with_spotter": false, "to_failure": false, "notes": ""},
    {"set_number": 2, "reps": 10, "weight": 0, "unit": "kg", 
     "with_spotter": false, "to_failure": false, "notes": ""},
    {"set_number": 3, "reps": 10, "weight": 0, "unit": "kg", 
     "with_spotter": false, "to_failure": false, "notes": ""},
    {"set_number": 4, "reps": 0, "weight": 0, "unit": "kg", 
     "with_spotter": true, "to_failure": true, 
     "notes": "failure set with spotter"}
  ]
}

"lat pulldown 3 sets 15 reps 50kg"
→ {
  "name": "Lat Pulldown",
  "muscle_group": "Back",
  "sets": [
    {"set_number": 1, "reps": 15, "weight": 50, "unit": "kg",
     "with_spotter": false, "to_failure": false, "notes": ""},
    {"set_number": 2, "reps": 15, "weight": 50, "unit": "kg",
     "with_spotter": false, "to_failure": false, "notes": ""},
    {"set_number": 3, "reps": 15, "weight": 50, "unit": "kg",
     "with_spotter": false, "to_failure": false, "notes": ""}
  ]
}
